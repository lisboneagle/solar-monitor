...
def is_success(data):
    """Deye uses code 1000000 for success (not 0 or 200)."""
...
def post(path, payload, token=None, params=None):
    ...
    if token:
        # token already contains "Bearer xyz" from the API response
        headers["Authorization"] = token if token.startswith("Bearer ") else f"Bearer {token}"
    resp = requests.post(f"{BASE_URL}/{path}", json=payload, headers=headers,
                         params=params, timeout=30)
    resp.raise_for_status()
    data = resp.json()
    print(f"    → /{path} response code: {data.get('code')} success: {data.get('success')}")
...
# The accessToken from Deye already includes "Bearer " prefix
